...
function Institucional() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let fotoURL = '';
      if (foto) {
        const photoRef = ref(storage, `fotos-institucionales/${user.uid}/${Date.now()}.jpg`);
        await uploadBytes(photoRef, foto);
        fotoURL = await getDownloadURL(photoRef);
      }

      await addDoc(collection(db, 'fichas_institucionales'), {
        ...form,
        fotoURL,
        usuarioId: user.uid,
        email: user.email,
      ...
      alert('Ficha institucional creada correctamente ✅');
      navigate('/menu');
    } catch (error) {
  ...
  return (
    <div className="form-container">
      <h2>Crear Ficha Institucional</h2>
